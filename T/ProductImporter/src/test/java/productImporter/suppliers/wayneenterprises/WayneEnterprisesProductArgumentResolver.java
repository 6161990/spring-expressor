...
public class WayneEnterprisesProductArgumentResolver implements DomainArgumentResolver {
    ...
    public Optional<Object> tryResolve(Class<?> parameterType) {
        if(parameterType.equals(WayneEnterprisesProduct.class)) { /** WayneEnterprisesProduct 타입이거나*/
            return Optional.of(generate());
        } else if (parameterType.equals(WayneEnterprisesProduct[].class)) { /** WayneEnterprisesProduct 배열 타입이면, 테스트 인자를 해결해준다*/
